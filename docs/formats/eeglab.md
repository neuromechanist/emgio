...
- An epoched MATLAB v7.3 file (`EEG.trials > 1`) raises `NotContinuousRecordingError` rather than being read as a fake continuous recording. (The classic path's pre-existing, separate behavior for an epoched `.fdt` is unchanged: it concatenates trials into one continuous series, since that is the tested, deliberate design for that case -- see `test_eeglab_reads_epoched_fdt`.)
